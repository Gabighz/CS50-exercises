...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // TODO: validate request-line (CHANGE THIS)
            int first_space = 0, second_space = 0;
            for(int i = 0; i < strlen(line); i++)
            {
             if (line[i] == ' ' && first_space == 0)
             {
               first_space = i;
             }
             if (line[i] == ' ' && first_space != 0)
            ...
            char* request_target = malloc(sizeof(line));
            for (int j = 0, n = second_space - first_space; j < n; j++)
            {
             request_target[j] = line[j];
            } 
            ...
            for (int i = 0, n = second_space - first_space; i < n; i++)
            {
               if(request_target[i] == '"')
               {
                 error(400);
            ...
            char HTTP_version[9];
            for (int i = 0, j = second_space + 1; i < 8; i++, j++)
            {
             HTTP_version[i] = line[j];             
            }
            HTTP_version[8] = '\0';
            ...
            int lenght_absolute_path = 0;
            
            for (int i = first_space + 1; i < second_space; i++)
            {
             lenght_absolute_path++;
             if(line[i] == '.')
             {
              extension_index = i;
            ...
            // TODO: extract query from request-target
             char query[(second_space - start_of_query) + 1];
             if((start_of_query != 0) && (start_of_query != second_space))
             {
              for (int i = start_of_query, j = 0; i < second_space; i++, j++)
              {
               query[j] = line[i];
              }
              query[second_space - start_of_query] = '\0';
             }
             if((start_of_query == 0) || (start_of_query == second_space))
             {
              query[0] = '\0';
            ...
            char path[strlen(root) + lenght_absolute_path +1];
            strcpy(path,root);
            for (int i = first_space + 1, j = strlen(root); i < first_space + lenght_absolute_path + 1; i++, j++)
            {
             path[j] = line[i];
            }
            path[strlen(root) + lenght_absolute_path] = '\0';
              
            
             
            // TODO: ensure path exists error(404)
           ...
           // TODO: ensure path is readable
            ...
            char extension[10];
            int markup_number;
            for(int i = first_space + (extension_index - first_space) + 1, j = 0; (line[i] != '?') && (i < second_space); i++, j++)
            {
             extension[j] = line[i];
             markup_number = j + 1;
            }
            extension[markup_number] = '\0';
